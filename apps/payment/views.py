    ...
    def get(self, request, format=None):
        ...
        try:
            ...
            if not CartItem.objects.filter(cart=cart).exists():
                return Response(
                    {'error': 'Necesito tener artículos en el carrito.'},
            ...
            for cart_item in cart_items:
                if not Product.objects.filter(id=cart_item.product.id).exists():
                    return Response(
                        {'error': 'Un producto con ID proporcionado no existe.'},
                        status=status.HTTP_404_NOT_FOUND
                    )
    ...
    def post(self, request, format=None):
        ...
        if newTransaction.is_success or newTransaction.transaction:
            ...
            try:
                ...
                return Response(
                    {'error': 'La transacción se realizó correctamente pero no se pudo crear el pedido.'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
            ...
            for cart_item in cart_items:
                try:
                    ...
                    return Response(
                        {'error': 'La transacción se realizó correctamente y se creó el pedido, pero no se pudo crear un artículo de pedido.'},
                        status=status.HTTP_500_INTERNAL_SERVER_ERROR
                    )
            ...
            try:
                ...
                return Response(
                    {'error': 'La transacción y el pedido se realizaron correctamente, pero no se pudo borrar el carrito.'},
                    status=status.HTTP_500_INTERNAL_SERVER_ERROR
                )
            
            return Response(
                {'success': 'La transacción fue exitosa y se creó el pedido.'},
                status=status.HTTP_200_OK
            )
        else:
            return Response(
